...
document.addEventListener("DOMContentLoaded", function () {
  /**
   * Extracts data from a table marked with the class 'scrollable-table' and
   * having a data attribute 'data-name' set to "mainScrollableTable".
   *
   * @returns {Object} An object containing arrays for candlestick data and volume.
   */
  function extractDataFromMainTable() {
    // Query the table using its class and data attribute.
    ...
    // Iterate through the rows, skipping the first row (headers).
    for (let i = 1; i < rows.length; i++) {
      const cells = rows[i].querySelectorAll("td");
      // Extract and process data from the table cells.
