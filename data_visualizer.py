...
class DataVisualizer(QObject):
    ...
    def plot_candlestick_chart(self, df=None, indicators=None):
        try:
            ...
            if indicators:
                ...
                if 'SMA' in indicators:
                    for sma_col in ['SMA', 'SMA50', 'SMA200']:
                        if sma_col in df.columns:
                            fig.add_trace(go.Scatter(x=df['Date'], y=df[sma_col], mode='lines', name=sma_col, line=dict(
                                color=sma_colors[sma_col], width=2.5)))
                            self.annotate_sma_events(fig, df)

                # RSI_14
                if 'RSI_14' in indicators and 'RSI_14' in df.columns:
                    fig.add_trace(go.Scatter(x=df['Date'], y=df['RSI_14'], mode='lines', name='RSI_14', yaxis='y2', line=dict(
                        color=secondary_color, width=2.5)))
                    self._add_rsi_guidelines_to_fig(fig, df)

                # RSI_25
                if 'RSI_25' in indicators and 'RSI_25' in df.columns:
                    fig.add_trace(go.Scatter(x=df['Date'], y=df['RSI_25'], mode='lines', name='RSI_25', yaxis='y2', line=dict(
                        color=tertiary_color, width=2.5)))
                    self._add_rsi_guidelines_to_fig(fig, df)

                # RSI_9
                if 'RSI_9' in indicators and 'RSI_9' in df.columns:
                    fig.add_trace(go.Scatter(x=df['Date'], y=df['RSI_9'], mode='lines', name='RSI_9', yaxis='y2', line=dict(
                ...
                if 'STOCH' in indicators:
                    if 'STOCHk_9_6_3' in df.columns:
                        fig.add_trace(go.Scatter(x=df['Date'], y=df['STOCHk_9_6_3'], mode='lines',
                                      name='STOCHk_9_6_3', yaxis='y2', line=dict(color=secondary_color, width=2.5)))
                    if 'STOCHd_9_6_3' in df.columns:
                        fig.add_trace(go.Scatter(x=df['Date'], y=df['STOCHd_9_6_3'], mode='lines',
                                      name='STOCHd_9_6_3', yaxis='y2', line=dict(color=tertiary_color, width=2.5)))
                    self._add_stoch_guidelines_to_fig(fig, df)

                # Chaikin Money Flow
                if 'CMF_20' in indicators and 'CMF_20' in df.columns:
                    fig.add_trace(go.Scatter(x=df['Date'], y=df['CMF_20'], mode='lines', name='CMF_20', yaxis='y2', line=dict(
                        color=quaternary_color, width=2.5)))

                # MACD
                if 'MACD_12_26_9' in indicators:
                    if 'MACD_12_26_9' in df.columns:
                        fig.add_trace(go.Scatter(x=df['Date'], y=df['MACD_12_26_9'], mode='lines',
                                      name='MACD_12_26_9', yaxis='y2', line=dict(color=secondary_color, width=2.5)))
                    if 'MACDs_12_26_9' in df.columns:
                        fig.add_trace(go.Scatter(x=df['Date'], y=df['MACDs_12_26_9'], mode='lines',
                                      name='MACDs_12_26_9', yaxis='y2', line=dict(color=tertiary_color, width=2.5)))
            ...
            self.logger.log_or_print(
                f"plot_candlestick_chart: ValueError encountered: {ve}", level='ERROR', exc_info=True)
        except KeyError as ke:
            self.logger.log_or_print(
                f"plot_candlestick_chart: KeyError encountered: {ke}", level='ERROR', exc_info=True)
        except Exception as e:
            self.logger.log_or_print(
                f"plot_candlestick_chart: Unexpected error encountered: {e}", level='ERROR', exc_info=True)
    ...
    def _add_rsi_guidelines_to_fig(self, fig, df):
        ...
        fig.add_shape(type="line", x0=df['Date'].iloc[0], x1=df['Date'].iloc[-1],
                      y0=70, y1=70, yref='y2', line=dict(color="blue", width=2))
        fig.add_shape(type="line", x0=df['Date'].iloc[0], x1=df['Date'].iloc[-1],
                      y0=30, y1=30, yref='y2', line=dict(color="blue", width=2))
    ...
    def _add_stoch_guidelines_to_fig(self, fig, df):
        ...
        fig.add_shape(type="line", x0=df['Date'].iloc[0], x1=df['Date'].iloc[-1],
                      y0=20, y1=20, yref='y2', line=dict(color="blue", width=2))
        fig.add_shape(type="line", x0=df['Date'].iloc[0], x1=df['Date'].iloc[-1],
                      y0=80, y1=80, yref='y2', line=dict(color="blue", width=2))
        ...
        try:
            ...
            for i, row in df.iterrows():
                ...
                if row['Price_Cross_SMA50_Up'] == 1:
                    fig.add_annotation(
                        x=x_val, y=row['Close'], text="Price crosses SMA50 Up", showarrow=True, arrowhead=2)
                elif row['Price_Cross_SMA50_Down'] == 1:
                    fig.add_annotation(
                        x=x_val, y=row['Close'], text="Price crosses SMA50 Down", showarrow=True, arrowhead=2)

                if row['Price_Cross_SMA200_Up'] == 1:
                    fig.add_annotation(
                        x=x_val, y=row['Close'], text="Price crosses SMA200 Up", showarrow=True, arrowhead=2)
                elif row['Price_Cross_SMA200_Down'] == 1:
                    fig.add_annotation(
                        x=x_val, y=row['Close'], text="Price crosses SMA200 Down", showarrow=True, arrowhead=2)
            ...
            self.logger.log_or_print(
                f"annotate_sma_events: KeyError encountered: {ke}", level='ERROR', exc_info=True)
        except Exception as e:
            self.logger.log_or_print(
                f"annotate_sma_events: Unexpected error encountered: {e}", level='ERROR', exc_info=True)
