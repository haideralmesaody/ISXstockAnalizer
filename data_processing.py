...
class DataProcessing:
    ...
    def plot_candlestick_chart(self, df=None):
        try:
            print("plot_candlestick_chart: Function called.")
            
            if df is None:
                raise ValueError("No DataFrame provided for plotting.")
                
            print("plot_candlestick_chart: DataFrame is available.")
            ...
            if self.main_window.sma_checkbox.isChecked():
                print("plot_candlestick_chart: SMA Checkbox is checked.")
            ...
            if self.main_window.rsi_checkbox.isChecked() and 'RSI14' in df.columns:
                print("plot_candlestick_chart: RSI Checkbox is checked.")
            ...
            if self.main_window.stoch_checkbox.isChecked() and '%K' in df.columns and '%D' in df.columns:
                print("plot_candlestick_chart: Stoch Checkbox is checked.")
                fig.add_trace(go.Scatter(x=df['Date'], y=df['%K'], mode='lines', name='%K', yaxis='y2'))
                fig.add_trace(go.Scatter(x=df['Date'], y=df['%D'], mode='lines', name='%D', yaxis='y2'))
            ...
            print(f"plot_candlestick_chart: ValueError encountered: {ve}")
            logging.error(f"ValueError in plot_candlestick_chart: {ve}")
            self.main_window.statusBar().showMessage(str(ve))
        except KeyError as ke:
            print(f"plot_candlestick_chart: KeyError encountered: {ke}")
            logging.error(f"KeyError in plot_candlestick_chart: {ke}")
            self.main_window.statusBar().showMessage(f"Missing key in DataFrame: {ke}")
        except Exception as e:
            print(f"plot_candlestick_chart: Unexpected error encountered: {e}")
            logging.error(f"An unexpected error occurred in plot_candlestick_chart: {e}")
